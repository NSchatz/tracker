...
| **A thief with an enrolled phone** | **Partly** | The device token is the credential; it can be **rotated** and given an **expiry** so a lost phone's token stops working. On the client, the token is held in encrypted storage (client track). A device token can only *write its own* fixes, never read the family. **Since C2 the phone also buffers undelivered fixes on disk** (app-private `filesDir`, `allowBackup="false"`, bounded and emptied as the server confirms them) — so a phone that has been offline holds a short window of its own recent coordinates in the clear, readable with root or an unlocked bootloader, exactly as the token already is. |
